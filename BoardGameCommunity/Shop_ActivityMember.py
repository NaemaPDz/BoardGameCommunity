...
# Form implementation generated from reading ui file 'UI/Shop_ActivityMember.ui'
#
...
class Ui_Dialog(object):
    def setupUi(self, Dialog):
        ...
        self.tbl_activitiyMember = QtWidgets.QTableWidget(Dialog)
        self.tbl_activitiyMember.setGeometry(QtCore.QRect(20, 160, 431, 491))
        self.tbl_activitiyMember.setStyleSheet("font: 10pt \"Bai Jamjuree\";\n"
                                               "background-color: rgb(255, 255, 255);")
        self.tbl_activitiyMember.setObjectName("tbl_activitiyMember")
        self.tbl_activitiyMember.setColumnCount(0)
        self.tbl_activitiyMember.setRowCount(0)

        self.retranslateUi(Dialog)
        QtCore.QMetaObject.connectSlotsByName(Dialog)

        self.this_dialog = Dialog
        self.SetActivityDetail()
        self.DisplayActivityMember()

        self.btn_back.clicked.connect(self.BackToActivityDetail)

    def __init__(self, passData):
        self.activityData = passData

    def SetActivityDetail(self):
        self.lbl_title.setText(self.activityData['activityName'])
        self.lbl_member.setText(
            "{} / {}".format(self.activityData['joinedPlayerID'].__len__(), self.activityData['maxPlayer']))

    def DisplayActivityMember(self):
        memberList = am.FetchPlayerList({'_id': {'$in': self.activityData['joinedPlayerID']}})

        self.tbl_activitiyMember.setRowCount(memberList.__len__())
        self.tbl_activitiyMember.setColumnCount(2)

        self.tbl_activitiyMember.setHorizontalHeaderItem(0, QTableWidgetItem("USERNAME"))
        self.tbl_activitiyMember.setHorizontalHeaderItem(1, QTableWidgetItem("TELEPHONE"))

        row = 0
        for i in memberList:
            self.tbl_activitiyMember.setItem(row, 0, QTableWidgetItem("{}".format(i['username'])))
            self.tbl_activitiyMember.setItem(row, 1, QTableWidgetItem("{}".format(i['phone'])))
            row += 1

    def BackToActivityDetail(self):
